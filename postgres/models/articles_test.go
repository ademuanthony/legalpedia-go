...
func testArticlesExists(t *testing.T) {
	...
	e, err := ArticleExists(ctx, tx, o.UUID)
	if err != nil {
...
func testArticlesFind(t *testing.T) {
	...
	articleFound, err := FindArticle(ctx, tx, o.UUID)
	if err != nil {
		t.Error(err)
	}
...
var (
	articleDBTypes = map[string]string{`UUID`: `character`, `Title`: `text`, `Content`: `text`, `VersionNo`: `integer`, `ID`: `integer`}
	_              = bytes.MinRead
)
