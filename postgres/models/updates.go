...
var UpdateWhere = struct {
	...
	Rowid:  whereHelperint64{field: "\"updates\".\"rowid\""},
	Type:   whereHelpernull_String{field: "\"updates\".\"type\""},
	Count:  whereHelpernull_Int{field: "\"updates\".\"count\""},
	Status: whereHelpernull_Int{field: "\"updates\".\"status\""},
}
...
var (
	updateAllColumns            = []string{"rowid", "type", "count", "status"}
	updateColumnsWithoutDefault = []string{"rowid", "type", "count", "status"}
	updateColumnsWithDefault    = []string{}
	updatePrimaryKeyColumns     = []string{"rowid"}
)
...
func FindUpdate(ctx context.Context, exec boil.ContextExecutor, rowid int64, selectCols ...string) (*Update, error) {
	updateObj := &Update{}
	...
	query := fmt.Sprintf(
		"select %s from \"updates\" where \"rowid\"=$1", sel,
	)
...
func (o *Update) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"updates\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, updatePrimaryKeyColumns),
		)
...
func (o UpdateSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"updates\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, updatePrimaryKeyColumns, len(o)))
...
func (o *Update) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"updates\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Update) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"updates\" WHERE \"rowid\"=$1"
...
func (o UpdateSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"updates\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, updatePrimaryKeyColumns, len(o))
...
func (o *Update) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUpdate(ctx, exec, o.Rowid)
	if err != nil {
		return err
	}
...
func UpdateExists(ctx context.Context, exec boil.ContextExecutor, rowid int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"updates\" where \"rowid\"=$1 limit 1)"
