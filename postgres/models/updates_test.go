...
func testUpdatesExists(t *testing.T) {
	...
	e, err := UpdateExists(ctx, tx, o.Rowid)
	if err != nil {
...
func testUpdatesFind(t *testing.T) {
	...
	updateFound, err := FindUpdate(ctx, tx, o.Rowid)
	if err != nil {
		t.Error(err)
	}
...
var (
	updateDBTypes = map[string]string{`Rowid`: `bigint`, `Type`: `text`, `Count`: `integer`, `Status`: `integer`}
	_             = bytes.MinRead
)
