...
// HoldenAt is an object representing the database table.
type HoldenAt struct {
	ID       int         `boil:"Id" json:"Id" toml:"Id" yaml:"Id"`
	HoldenAt null.String `boil:"HoldenAt" json:"HoldenAt,omitempty" toml:"HoldenAt" yaml:"HoldenAt,omitempty"`

	R *holdenAtR `boil:"-" json:"-" toml:"-" yaml:"-"`
	L holdenAtL  `boil:"-" json:"-" toml:"-" yaml:"-"`
}

var HoldenAtColumns = struct {
	ID       string
	HoldenAt string
}{
	ID:       "Id",
...
var HoldenAtWhere = struct {
	ID       whereHelperint
	HoldenAt whereHelpernull_String
}{
	ID:       whereHelperint{field: "\"HoldenAt\".\"Id\""},
	HoldenAt: whereHelpernull_String{field: "\"HoldenAt\".\"HoldenAt\""},
}

// HoldenAtRels is where relationship names are stored.
var HoldenAtRels = struct {
}{}

// holdenAtR is where relationships are stored.
type holdenAtR struct {
}
...
// holdenAtL is where Load methods for each relationship are stored.
type holdenAtL struct{}

var (
	holdenAtAllColumns            = []string{"Id", "HoldenAt"}
	holdenAtColumnsWithoutDefault = []string{"Id", "HoldenAt"}
	holdenAtColumnsWithDefault    = []string{}
	holdenAtPrimaryKeyColumns     = []string{"Id"}
)

type (
	// HoldenAtSlice is an alias for a slice of pointers to HoldenAt.
	// This should generally be used opposed to []HoldenAt.
	HoldenAtSlice []*HoldenAt

	holdenAtQuery struct {
		*queries.Query
	}
)
...
var (
	holdenAtType                 = reflect.TypeOf(&HoldenAt{})
	holdenAtMapping              = queries.MakeStructMapping(holdenAtType)
	holdenAtPrimaryKeyMapping, _ = queries.BindMapping(holdenAtType, holdenAtMapping, holdenAtPrimaryKeyColumns)
	holdenAtInsertCacheMut       sync.RWMutex
	holdenAtInsertCache          = make(map[string]insertCache)
	holdenAtUpdateCacheMut       sync.RWMutex
	holdenAtUpdateCache          = make(map[string]updateCache)
	holdenAtUpsertCacheMut       sync.RWMutex
	holdenAtUpsertCache          = make(map[string]insertCache)
)
...
// One returns a single holdenAt record from the query.
...
// All returns all HoldenAt records from the query.
...
// Count returns the count of all HoldenAt records in the query.
...
// HoldenAts retrieves all the records using an executor.
func HoldenAts(mods ...qm.QueryMod) holdenAtQuery {
	mods = append(mods, qm.From("\"HoldenAt\""))
	return holdenAtQuery{NewQuery(mods...)}
}

// FindHoldenAt retrieves a single record by ID with an executor.
...
// Update uses an executor to update the HoldenAt.
...
// Delete deletes a single HoldenAt record with an executor.
...
// HoldenAtExists checks if the HoldenAt row exists.
func HoldenAtExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"HoldenAt\" where \"Id\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if HoldenAt exists")
	}
