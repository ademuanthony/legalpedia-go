...
var (
	noteDBTypes = map[string]string{`ID`: `text`, `TeamId`: `text`, `CreatorId`: `bigint`, `Title`: `text`, `Summary`: `text`, `Body`: `text`, `Image`: `text`, `AccessType`: `integer`, `Labels`: `text`, `CreatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
...
func testNotesUpdate(t *testing.T) {
	...
	if len(noteAllColumns) == len(notePrimaryKeyColumns) {
...
func testNotesSliceUpdateAll(t *testing.T) {
	...
	var fields []string
	if strmangle.StringSliceMatch(noteAllColumns, notePrimaryKeyColumns) {
		fields = noteAllColumns
	} else {
