...
var (
	customerDBTypes = map[string]string{`ID`: `integer`, `Name`: `text`, `Surname`: `text`, `DateOfBirth`: `timestamp without time zone`, `PhoneNumber`: `text`, `Email`: `text`, `LastModificationTime`: `timestamp without time zone`, `LastModifierUserId`: `bigint`, `CreationTime`: `timestamp without time zone`, `CreatorUserId`: `bigint`, `TenantId`: `integer`, `City`: `text`, `State`: `text`, `Country`: `text`}
	_               = bytes.MinRead
)
